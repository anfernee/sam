...
func callMCP(t *testing.T, mcpAddr string, toolName string, params map[string]any) string {
	...
	session, err := client.Connect(ctx, &mcp.StreamableClientTransport{Endpoint: "http://" + mcpAddr + "/"}, nil)
	if err != nil {
...
func TestCatalogRoutingAndFailover(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		session, err := client.Connect(context.Background(), &mcp.StreamableClientTransport{Endpoint: "http://" + mcpAddrA + "/"}, nil)
		if err != nil {
